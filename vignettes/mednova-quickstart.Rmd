...
vignette: >
  %\VignetteIndexEntry{MedNova 快速开始}
...
MedNova 是一个面向医学研究任务的智能分析平台，用于把“数据对象 + 任务描述”转换为流程建议、函数说明与可执行 R 脚本。平台不会自动执行分析，而是把任务规划与代码生成整理得更直接。
...
- `med_psm()`、`med_logistic()`、`med_cox()`、`med_bulk_deg()`、`med_mr()`、`med_diagnostic_model()`：统一分析入口
- `mednova_task_assistant()`：任务与脚本入口
- `mednova_generate_script()`：脚本生成入口
- `run_mednova_app()`：平台中的交互式工作台入口
- MedNova Docs：平台文档中心
- MedNova Cases：平台案例与学习中心

# 输入与输出

`mednova_task_assistant()` 的最小输入包括：
...
如果列名不标准，建议通过 `spec` 或 MedNova Studio 显式指定列角色。平台优先把“用户明确指定”视为最终结论。
...
MedNova 会根据任务匹配结果，从内置函数目录中筛出当前任务优先推荐的函数，并给出：
...
可以。MedNova 会使用最小启发式规则推断部分字段，但更推荐显式指定关键列角色。
